...
#### [Huisman and Weissing (1990) Biodiversity of plankton by species oscillations and chaos. *Nature*, 402, 407–410](https://www.nature.com/articles/46540)
...
```{r}
m1 <- sim_rescomp(pars, y = c(1000, 1000, amp))
```


```{r}
plot_m1 <- plot_rescomp(m1, resource = FALSE, logy = TRUE)
```
...
pars$respulse <- amp
m4 <- sim_rescomp(pars, y = c(1000, 1000, amp))
...
pars$pulsefreq <- period
pars$respulse <- amp
m10 <- sim_rescomp(pars, y = c(1000, 1000, amp))
...
pars$respulse <- 0
pars$resconc <- 0.0002
mcons <- sim_rescomp(pars, y = c(1000, 1000, amp))
plot_rescomp(mcons, resource = FALSE, logy = TRUE)
```
