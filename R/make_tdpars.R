...
make_tdpars <- function(crpars, pars){
  if (is.vector(eval(parse(text = paste0("pars$", crpars)))[[1]])){
    if (pars$tpinterp == "inst" | pars$tpinterp == "lin"){
      ...
      for (i in seq_len(length(pars[[crpars]][[1]]))) {
        force_cr <- rep(c(pars[[crpars]][[1]][i],
                          pars[[crpars]][[2]][i]),
                        length.out = length(forcetime))
        cr_funs_bycons[[i]] <- approxfun(forcetime,
                                        force_cr,
                                        method = interpmethod,
                                        rule = 2)
      }
      ...
      for (i in seq_len(length(pars[[crpars]][[1]]))) {
        amplitude <- (pars[[crpars]][[1]][i] - pars[[crpars]][[2]][i])/2
        meancr <- (pars[[crpars]][[1]][i] + pars[[crpars]][[2]][i])/2
        wave <- (meancr + amplitude*sin((2*pi*timeseq)/(pars$timeparfreq)/2))
        cr_funs_bycons[[i]] <- approxfun(timeseq,
                                        wave,
                                        method = "linear",
                                        rule = 2)
        }
  ...
  } else if (is.matrix(eval(parse(text = paste0("pars$", crpars)))[[1]])){
    if (pars$tpinterp == "inst" | pars$tpinterp == "lin"){
      forcetime <- seq(0, pars$totaltime, pars$timeparfreq)
      cr_funs_byres <- list()
      cr_funs_bycons <- list()
